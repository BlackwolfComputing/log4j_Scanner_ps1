$starttime = get-date 
#gwmi win32_volume | where-object { $_.filesystem -match "ntfs" -and $_.name -match "^[A-Z]:" } | sort { $_.name } 
$Drives = (gwmi win32_volume | where-object { $_.filesystem -match "ntfs" -and $_.name -match "^[A-Z]:" } | sort { $_.name }).name #Can be turned to a mapped shared location 
...
$log = '\\B20000.bec.dk\fit2\App\Logging\Nessus\log4j\Log4jScanner-Results' 
$hostname = Hostname #will become ScanResults File name 
...
$FindingsTab = @() 
$vulnerablelist = $hash #$(Get-Content "$log\hash.txt") 
$localsums = $null 
$vulnerablesums = (-Split $vulnerablelist |? {$_.length -eq 64}) 

  
$VulnerableFiles = ((-Split $vulnerablelist |? {$_.length -ne 64})|%{$_.Replace('./','').Split('/')[-1]}) 
    foreach ($drive in $Drives){ 
     ...
     $ScanFiles = get-childitem -path "$drive" -Recurse | ?{$_.Extension -in ".jar",".war",".ear"} -ErrorAction SilentlyContinue | Get-FileHash 
        ...
        $file.Hash 
        $file.Path 
        $Test = $((Compare-Object $file.Hash -DifferenceObject $vulnerablesums -IncludeEqual -ErrorAction SilentlyContinue).SideIndicator -eq "==") 
        if ($test<#$file.Path -like "*log4j*.jar"#>){ #LOG4J USED by HASH 
             
            $Vuln = ($vulnerablelist -like "*$($file.Hash)*") 
            AddToObject $file 'Vulnerable' 'True' 
            AddToObject $file 'Version' "$($Vuln.Replace($file.Hash,''))" 
            $FindingsTab += Entry $file.Path "JAR-Vulnerable" $file.Version 
...
$FindingsTab | % {$_.Version = ($_.Version).Trim();$analysis = $(if ((([int]($_.Version).Split(' ')[-2] -gt '2014') -and (($_.Version -notlike "*log4j-1*"))) -or($_.Version -like "*log4j2*") -or ($_.Version -like "*log4j-2*") -or ($_.Version -like "*log4j-2*")){"Potentially Vulnerable Version2 or included post 2014: Log4j v2 release"}else{"Seems non-vulnerable. TBC"}); 
  AddToObject $_ 'Analysis' $analysis      } 
  
  
if ($FindingsTab.count -eq 0){"No Vulnerabilities found" | Out-File "$log\$hostname WAR.log"} 
else { 
$FindingsTab | Export-Csv -Path "$log\$hostname WAR.csv" -NoTypeInformation -Force 
} 

  
$Error |Select Exception | Export-Csv -Path "$log\$hostname WAR-Errors.csv"  -NoTypeInformation -Force 
$Executiontime = "$([Math]::Round(($(get-date) - $starttime).TotalMinutes,2)) minutes" 
$Executiontime
